...
def execute(Map config) {
    ...
    def groupTag = config.groupTag ?: 'default'        // <<< 新增:防序列多worker數時同BUILD_ID覆寫finalLog
...
""") {
                // <<< 改:pod yaml 兩處變動 —— (1)新增anti-affinity(與並行版一致,讓四演算法條件對等) (2)cpu改用${thisCpu}
                node(podLabel) {
                    container('defects4j') {
...
"""
    ...
    node('built-in') {
        def finalLog = "${frameworkPath}/experiments/${algorithmName}/batch_durations_${BUILD_ID}_${groupTag}.log"   // <<< 改:加_${groupTag}防序列多worker數覆寫
        sh "touch ${finalLog}"
